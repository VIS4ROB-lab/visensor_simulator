...
// error in 0.2rad(=11.5deg) and meters- these numbers need to be proportinal to the noise in the odometry used as input.
    ...
    bool getNextWaypoint( Eigen::Vector3d &desired_position, double &desired_yaw, float &desired_gimbal_pitch)  override;
 
    bool step( const geometry_msgs::Pose& curr_pose) override;
