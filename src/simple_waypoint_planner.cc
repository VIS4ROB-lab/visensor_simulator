...
SimpleWaypointPlanner::SimpleWaypointPlanner()
    : SimpleWaypointPlanner(0.2, 2) {}
...
SimpleWaypointPlanner::SimpleWaypointPlanner(
  ...
  robot_odometry_sub_ = nh_.subscribe(
      ...
      &SimpleWaypointPlanner::robotOdometryCallback, this);
...
void SimpleWaypointPlanner::robotOdometryCallback(
    const nav_msgs::Odometry& curr_odometry) {
  if (status_ == STARTING) {
    if (reachedNextWaypoint(curr_odometry.pose.pose)) {
      status_ =
          RUNNING;  // do not send waypoints yet, the logger need to be started
    } else {
      goNextPose();
    }
...
bool SimpleWaypointPlanner::getNextWaypoint(
    Eigen::Vector3d& desired_position, double& desired_yaw,
    float& desired_gimbal_pitch) {
  if (waypoints_.size() > 0) {
    Eigen::Vector3d position(
        waypoints_.front().x, waypoints_.front().y, waypoints_.front().z);
...
bool SimpleWaypointPlanner::reachedNextWaypoint(
    const geometry_msgs::Pose&
        curr_pose)  // do not consider the gimbal position
{
  if (waypoints_.size() == 0)
    return true;  // nowhere to go

  double position_error_squared =
      squared_dist(curr_pose.position, waypoints_.front());
  double yaw = quat2yaw(curr_pose.orientation);
  const static double c_2pi = 2 * M_PI;
  double yaw_error =
      std::fmod((std::abs(waypoints_.front().yaw - yaw) + c_2pi), c_2pi);
  // ROS_ERROR_STREAM("DEGUB error :" << position_error_squared << " "<<
  // position_max_error_squared_ << " "<< yaw_error << " "<< yaw_max_error_);
  if (position_error_squared < position_max_error_squared_ &&
      ((yaw_error < yaw_max_error_) ||
       (yaw_error > (c_2pi - yaw_max_error_)))) {
    return true;
  }
  return false;
}

void SimpleWaypointPlanner::sendPoseCommand(
    const Eigen::Vector3d& desired_position, const double& desired_yaw,
    const float& desired_gimbal_pitch) {
  trajectory_msgs::MultiDOFJointTrajectory trajectory_msg;
  trajectory_msg.header.stamp = ros::Time::now();
  mav_msgs::msgMultiDofJointTrajectoryFromPositionYaw(
      desired_position, desired_yaw, &trajectory_msg);
  ROS_INFO(
      "Publishing waypoint: %0.1lf :: %0.1lf :: %0.1lf :: %0.1lf :: %0.1f",
      desired_position.x(), desired_position.y(), desired_position.z(),
      desired_yaw / kDEG_2_RAD, desired_gimbal_pitch);
